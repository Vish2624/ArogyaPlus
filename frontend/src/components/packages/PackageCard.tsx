...
export default function PackageCard({ pkg, onViewDetails }: PackageCardProps) {
  ...
  return (
    <div
      className={`card relative flex h-[600px] flex-col p-6 hover:-translate-y-1 hover:scale-[1.02] hover:shadow-hover hover:ring-primary-500/40 ${pkg.is_featured ? "ring-2 ring-primary-500" : ""}`}
    >
      {pkg.is_featured && (
      ...
      <div className="mb-4 flex items-center gap-3">
        {pkg.image_url ? (
          <span className="flex h-11 w-11 shrink-0 overflow-hidden rounded-full ring-1 ring-slate-200">
            <img src={pkg.image_url} alt="" className="h-full w-full object-cover" />
          </span>
        ) : (
          <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${style.bg} ${style.text}`}>
            <Icon className="h-5 w-5" aria-hidden="true" />
          </span>
        )}
        {pkg.category && <span className={`badge w-fit ${style.bg} ${style.text}`}>{pkg.category}</span>}
      </div>
      <h3 className="line-clamp-1 text-card-title font-bold text-slate-900">{pkg.name}</h3>
      {pkg.description && <p className="mt-2 line-clamp-2 text-base leading-7 text-slate-500">{pkg.description}</p>}
      ...
      <ul className="custom-scrollbar mt-3 max-h-40 flex-1 space-y-2 overflow-y-auto border-t border-slate-100 pr-2 pt-3 text-sm text-slate-600">
        {pkg.tests.map((test) => (
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 border-t border-slate-100 pt-4">
        <div>
          ...
          <p className="text-slate-900">
            <span className="text-sm font-semibold text-slate-500">{labPrice.currency}</span>{" "}
            <span className="text-xl font-extrabold">{labPrice.amount}</span>
          </p>
        </div>
        <div>
          <p className="text-xs text-slate-500">Home Visit</p>
          <p className="text-slate-900">
            <span className="text-sm font-semibold text-slate-500">{homePrice.currency}</span>{" "}
            <span className="text-xl font-extrabold">{homePrice.amount}</span>
          </p>
        </div>
      </div>

      <div className="mt-6 flex gap-2">
        <button type="button" onClick={() => onViewDetails(pkg)} className="btn-secondary flex-1 !px-3">
